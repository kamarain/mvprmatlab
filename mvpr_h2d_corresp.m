...
% See also .
%
function [H] = mvpr_h2d_corresp(X1_,X2_,varargin)
 ...
 case 'isometry',
  switch conf.estMethod,
   case {'DEFAULT','umeyama-iso'},
    H = mvpr_hnd_corresp_umeyama(X1(1:2,:), X2(1:2,:), 1);
   case 'ransam_isometry',
    H = mvpr_h2d_corresp_ransam(X1, X2, 1, conf.ransam_iters, 1);
   case 'exact_isometry',
    H = mvpr_h2d_corresp_exiso(X1(1:2,:), X2(1:2,:));
   otherwise,
 ...
 case 'similarity',
  switch conf.estMethod,
   case {'DEFAULT','umeyama'},
    H = mvpr_hnd_corresp_umeyama(X1(1:2,:), X2(1:2,:), 0);
   case 'ransam_similarity',
    H = mvpr_h2d_corresp_ransam(X1, X2, 2, conf.ransam_iters, 1);
   case 'exact_similarity',
    H = mvpr_h2d_corresp_exsim(X1(1:2,:), X2(1:2,:));
   case 'dlt_r1',
    [nX1 nX2 Un Tn] = mvpr_hnd_corresp_coordnorm(X1(1:2,:),X2(1:2,:),conf.dlt_normMeth);
    nX1 = [nX1; X1(3,:)];
    nX2 = [nX2; X2(3,:)];
    nH = mvpr_h2d_corresp_dlt(nX1, nX2,1);
 ...
 case 'affinity',
  switch conf.estMethod,
   case {'DEFAULT','dlt_r2'},
    ...
    nX1 = [nX1; X1(3,:)];
    nX2 = [nX2; X2(3,:)];
    nH = mvpr_h2d_corresp_dlt(nX1,nX2,2);
    H = mvpr_hnd_corresp_coorddenorm(nH, Un, Tn);
   case 'ransam_affinity',
    H = mvpr_h2d_corresp_ransam(X1, X2, 3, conf.ransam_iters, 1);
   case 'exact_affinity',
    H = mvpr_h2d_corresp_exaff(X1(1:2,:), X2(1:2,:));
   otherwise,
    error(['Not supported estimation method for the homography of ' ...
           'type ' conf.hType]);
  end;
 ...
 case 'projectivity',
  switch conf.estMethod,
   ...
   case 'dlt_r3',
    ...
    nX1 = [nX1; X1(3,:)];
    nX2 = [nX2; X2(3,:)];
    nH = mvpr_h2d_corresp_dlt(nX1,nX2,3);
    H = mvpr_hnd_corresp_coorddenorm(nH, Un, Tn);
   otherwise,
    error(['Not supported estimation method for the homography of ' ...
           'type ' conf.hType]);
